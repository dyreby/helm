...
Helm is a companion to the collaboration framework. Where the framework defines *how I think about collaboration*, Helm defines *how I practice it* in a terminal.
...
The agent is a stateless reasoning coprocessor — any LLM that can receive structured context and return structured proposals. Helm doesn't depend on a specific coding agent; it needs a thin interface for sending prompts and parsing responses. The agent proposes. Helm enforces. I approve. No tool autonomy. No hidden state. No silent scope expansion.

Sessions are short by design. The ideal session is one command, a few confirmations, and a clean exit. Over time, most runs should converge toward: take bearing, confirm, confirm, done.
...
Observe the world and orient. This is grounding.

Helm executes a `BearingPlan` — reading from the world across one or more domains (Files, GitHub, Search, Web). Each domain supports two levels of attention:
...
The result is a `Bearing`: the plan that produced it, a `WorldSnapshot` of what was observed, and a `Position` — a short, plain-text statement about the state of the world, generated by the agent from the full history of bearings in the session.

Bearings are deterministic given the same plan and world state. Position is the one place the agent's interpretation enters during this phase, and it's clearly labeled as such.

### Correct Course

Think. This is the only conversational phase.
...
An `ActionPlan` describes intent: post a review, open a PR, apply a patch, run a command. Before execution, I approve through a gate:
...
Nothing is overwritten. Nothing is dropped. Each bearing is an immutable record of plan + snapshot + position. Each action report is an immutable record of plan + outcome.
...
- **Snapshots** (recent): raw observed payloads, kept for the last 5 bearings per session
...
- **Search** — pattern matching across text. Survey returns file/location hit lists. Inspect returns matches with context.
...
Modeled as a map from scope targets to an optional set of focus items. No focus means survey-only. Focus means inspect those items (in addition to surveying the scope). A generic container (`Map<S, Option<Set<F>>>`) keeps this consistent across kinds while allowing type-safe scope/focus per domain.
...
During **Take Bearing**, the agent receives the full history of bearings (plans + positions, not old snapshots) and produces a Position string — short, bounded, descriptive. It may reference historical trends if clearly relevant; otherwise it ignores history. Position describes; it never prescribes.
...
| **Bearing** | Immutable record: plan + snapshot + position | Settled |
...
| **WorldSnapshot** | Raw observed data + world stamps | Open — might become just "Snapshot" |
...
- **Not an autonomous agent.** The agent never drives. It advises within hard constraints. The menu waits for me.
