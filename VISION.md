...
An observation points the spyglass at a mark and records what was seen as a sighting. Each mark is a domain of observable reality (Files, GitHub, Search, Web, etc.) with two levels of attention:
...
Take as many observations as you want. Each is self-contained: a mark, a sighting, a timestamp. Most are glances — quick looks that get discarded. Some are worth keeping.

A bearing is sealed when you select the observations that matter and attach a reading — your interpretation of what you saw. Observations you took but didn't find useful are simply not included.

A reading is a short, plain-text interpretation of the state of the world, generated by the agent from the full history of bearings in the voyage. A sentence or two, readable at a glance. It exists so that collaboration has a grounded starting point (the agent doesn't have to re-derive "what's going on" from raw evidence each time) and so the Logbook tells a readable story: scanning readings across bearings shows how the voyage unfolded without replaying raw sightings.

A reading defaults to a single generation. If it feels wrong, I can challenge it and the agent re-generates. But a reading describes; it never prescribes.

Bearings are deterministic given the same marks and world state. The reading is clearly labeled as agent-generated interpretation.

Bearings exist for continuity. You take one when you'd need context if you had to stop and come back in a new session — the reading captures where things stand so the next session doesn't start from scratch. If a voyage finishes in one session, the completion summary is often the only record needed.

### Correct Reading

Refine the agent's interpretation. Sometimes the reading is wrong, or it misses something I know. I can challenge it, add context ("that CI failure is a flake"), or guide the agent toward a more accurate statement.

This can involve conversation, editing, or both. The goal is a reading I trust before deciding what to do next.
...
The agent can see the current bearing and the history of all prior bearings (observations + readings), but never raw sightings from old observations. If it needs something from a prior scope, it must propose a new observation.

This phase ends when I have either new marks to observe (take another bearing) or an `ActionPlan` (ready to act). If the result is new marks, Helm immediately takes those observations and returns to the menu with an updated bearing.
...
Nothing is overwritten. Nothing is dropped. Each bearing is an immutable record of observations + reading. Each action report is an immutable record of plan + outcome.
...
Each mark describes a domain of observable reality — not a mechanism. Commands are how Helm fetches data; marks describe what Helm is looking at.
...
Inside each mark, attention is described using two concepts:
...
During **Take Bearing**, the agent receives the full history of bearings (observations + readings) and produces a reading. Short, bounded, descriptive. It may reference historical trends if clearly relevant; otherwise it ignores history. A reading describes; it never prescribes. I can challenge the reading if it's wrong, and the agent re-generates.

During **Correct Reading**, the agent re-generates the reading based on my feedback while staying grounded in the current bearing.

During **Correct Course**, the agent receives the current bearing, observation history, and constraints. It must return exactly one of: new marks to observe, an `ActionPlan`, or an abort. The conversation is bounded by this requirement.
...
| **Bearing** | Immutable record: observations + reading |
| **Observation** | Self-contained: mark + sighting + timestamp |
| **Mark** | What you pointed the spyglass at (a domain of observable reality) |
| **Sighting** | What you saw when observing a mark |
| **Reading** | Short text interpretation of what was observed (agent-generated) |
| **ActionPlan** | Intent to affect the world |
| **ActionReport** | Outcome of action, contains its plan |
| **Scope / Focus** | How a mark describes what to survey and what to inspect |
